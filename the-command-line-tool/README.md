...
To get the actual options of your Ampersand compiler, type

`ampersand --help`

This will give you an overview of the most popular options. If you want an overview of all available options, go for the verbose variant.

`ampersand --verbose --help`

## List of options:

* `--ECA` generate documentation with ECA rules.
  ...
  `--atlas-without-expressions`
          ...
          Temporary switch to create an Atlas without expressions. This feature is used in RAP3 for performance reasons.
  ...
  `--config=config.yaml`
          ...
          By default the Ampersand compiler looks in the working directory for a config file \(`.yaml`\) with the same filename as the Ampersand script. Use this feature to redirect to another file.
  ...
  `--crud-defaults=CRUD`
          ...
          Set the default CRUD function for interfaces. This function is explained in [interface expressions](../modeling-in-ampersand/interfaces.md). This feature is used temporarily to gain experience with CRUD and learn about its semantics.
