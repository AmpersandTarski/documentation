...
In the _**declaration**_ `RELATION owner[Person*Building]`, `owner` is the _**name**_, `[Person*Building]` is the _**type**_, and `owner[Person*Building]` is the **signature** of the relation. The left hand concept, `Person`, is called the source of the relation and the right concept, `Building`, is called the target.
